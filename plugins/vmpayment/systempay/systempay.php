...
class plgVMPaymentSystempay extends vmPSPlugin {
	...
	function __construct (& $subject, $config) {
		...
		$this->_tablepkey = 'id'; //virtuemart_systempay_id';
		$this->_tableId = 'id'; //'virtuemart_systempay_id';
	...
	function plgVmConfirmedOrder ($cart, $order) {
		...
		$form .= '<input type="image" name="submit" src="' . JURI::base (TRUE) . '/images/stories/virtuemart/payment/' . $this->_name . '.png" alt="' . vmText::_ ('VMPAYMENT_' . $this->_name . '_BTN_ALT') . '" title="' . vmText::_ ('VMPAYMENT_SYSTEMPAY_BTN_ALT') . '"/>';
	...
	function plgVmOnPaymentResponseReceived (&$html) {
		...
		$resp = new SystempayResponse($data, $method->ctx_mode, $method->key_test, $method->key_prod);
		...
		if (!$virtuemart_order_id) {
			...
			$html = $this->_getHtmlPaymentResponse ('VMPAYMENT_' . $this->_name . '_ERROR_MSG', FALSE);
			// 	vRequest::st::setVar('paymentResponseHtml', $html, 'post');
			return NULL;
		}
		...
		if ($order_status_code == 'P') {
			$this->logInfo ('plgVmOnPaymentResponseReceived -- check url does not work.', 'warning');
			 ...
			 if ($method->ctx_mode == 'TEST') {
				...
				$details .= '<a href="https://paiement.systempay.fr/html/faq/prod" target="_blank">https://paiement.systempay.fr/html/faq/prod</a>';
			...
			vmWarn (vmText::_ ('VMPAYMENT_' . $this->_name . '_SHOP_TO_PROD_INFO') . '<a href="https://paiement.systempay.fr/html/faq/prod" target="_blank">https://paiement.systempay.fr/html/faq/prod</a>', '');
		}
